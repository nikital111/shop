 ...
 const total = useSelector(state => state.total);
   ...
   return cart.map((product,i)=>{
    ...
    dispatch(setImg(product.imageURL));
     ...
     return(
       ...
       <div className='main'>
       {product.name}
       <img src={product.imageURL} alt={'Купить ' + product.name} onClick={zoomImg}/>
       </div>
       <div className='divCount'>
       <button className='delButt' onClick={delCart}>Убрать из корзины</button>
       {product.price*product.count + '$'}
       <input type='number' min='1' max='20' defaultValue={product.count} onChange={changeCount}/>
       </div>
       <hr />
       </div>
  ...
  return (
    ...
    <div className='emptyCart'>
      Корзина пуста
      <NavLink to='/' activeClassName=''>Вернуться в каталог</NavLink>
      </div>
      : 
      <div className='cart'>
